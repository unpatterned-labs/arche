...
- **Equivalence packs** *(data)* — [name equivalences](https://github.com/unpatterned-labs/arche/tree/main/datasets) across 50+ ethnic traditions, artist alias groups by genre. External knowledge that makes variants count as agreement. Recall lives here.
...
The one piece of inference opinion arche does hold is small and deliberate: the **distinctive-evidence gate** — no merge without at least one piece of distinctive agreement, which is how the Febrl4 person benchmark (synthetic, evaluated at the pairwise level where arche's shipped surface lives) holds [zero false merges at 0.877 auto-match recall](../tutorials/person_resolution_at_scale.md). Everything else defers: the in-memory Fellegi–Sunter core is intentionally thin, and at scale arche [stands on Splink](../tutorials/arche_vs_alternatives.md) rather than re-deriving it.
