...
- **Frequency tables** *(data)*. Measured token distinctiveness over real populations, loaded via `TokenFrequencyTable.default()` (persons) and `TokenFrequencyTable.default(domain="artist")` (a 500k-artist MusicBrainz sample). What agreement is *worth*. The tables feed the classical value-specific frequency adjustment (Winkler, 1989). Switching the signal off takes precision from 0.946 to 0.162 on [1,114 real same-surname pairs](the-whole-picture.md#the-frequency-row-used-to-say-something-else), and costs recall to buy it; a self-calibrated table on a small corpus [measurably fails](../tutorials/artist_resolution.md#3-why-the-shipped-table-matters-the-toy-corpus-trap) where the population table doesn't, though on that person benchmark self-calibration is the stronger of the two by F1.
...
*Models, weights, and grounding:* [Ditto](https://arxiv.org/abs/2004.00584) (Li et al., 2020), [DeepBlocker](https://vldb.org/pvldb/vol14/p2459-thirumuruganathan.pdf) (Thirumuruganathan et al., 2021), [Narayan et al.](https://arxiv.org/abs/2205.09911) (2022), and [Peeters & Bizer](https://arxiv.org/abs/2310.11244) (2023) measure what models can and can't do in matching; [Harnad (1990)](https://doi.org/10.1016/0167-2789(90)90087-6) frames the grounding problem; [Hogan et al. (2021)](https://arxiv.org/abs/2003.02320) survey the entity-centric (knowledge-graph) answer to it.
