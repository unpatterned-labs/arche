...
Three attributes were extracted, two of them identifying. `ex.record` holds the raw dict; `ex.reference` is the canonical form that feeds resolution.
...
The invented field is recorded and **excluded**. It never reaches the reference, and it is never coerced into a field that looks similar.
...
Extracted references flow into the same gated path as everything else. Pass `extra_pins=ex.pins(decl)` so the decision records what produced it:
...
`reproducible=False` because a model was in the chain. The same two references resolved by the engine alone attest as `reproducible=True`. A verifier can tell the two apart without trusting anyone's word for it.
...
The deterministic engine is the oracle. `grade_pairs` measures the model's judgment against it.
