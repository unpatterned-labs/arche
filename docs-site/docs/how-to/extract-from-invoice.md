...
Two things the older docs got wrong here: `Pipeline` takes no `audit_log=` argument (see the [constructor reference](../api/pipeline.md)), and `Result` has no `summary()` method - count over `result.detections` instead. To persist an audit trail to SQLite, see Persisted audit log.
