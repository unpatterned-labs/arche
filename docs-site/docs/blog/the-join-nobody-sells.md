...
It works when parties can be compelled to adopt a scheme, and retail, pharma, batteries and textiles (the UNTP pilot domains) are exactly those.
...
Three commodities, and the structure repeats. The node where the commodity is aggregated, and where a lot first acquires an identity, differs by commodity: a **society** for cocoa, a **washing station** for coffee, a **factory or estate** for tea. Its shape does not differ: a named site, operated by a legal entity, aggregating from many smallholders.
...
Writing that down took a day and was worth more than the code. The definition it belongs to, six entity classes and four outcomes and ten named failure cases, is the artifact a competitor cannot derive by reading the source.
...
The framing of the gap owes most to the people building the thing this argues is insufficient.

[GS1's](https://www.gs1.org/) identifier system and EPCIS, and the UN Transparency Protocol above them, are well designed and work as intended. The argument here is not that standardisation is wrong. It is that standardisation needs parties who can be compelled to adopt a scheme, and that condition fails at the smallholder edge.

[Trase](https://trase.earth/) publishes the supply-chain data this problem actually lives in, openly, covering Ghanaian and Ivorian cocoa among others. Their coverage is the population our own frequency table demonstrably lacks, and the next table should be built from it.

[GLEIF](https://www.gleif.org/en/about/open-data) releases legal-entity reference data under CC0, which is what made a first organisation frequency table possible at all. [OpenSanctions](https://www.opensanctions.org/docs/opensource/pairs/) has assembled the benchmark that would actually settle the accuracy question, and publishes it openly for non-commercial use.

The vendors named at the top (Meridia, TraceX, Koltiva, Farmerline, Satelligence) are solving a real problem competently. Nothing here says otherwise.
