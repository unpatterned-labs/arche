...
Two measurement caveats belong here rather than in a footnote. The place pack's headline **88.1% is a consistency figure, not accuracy**. OpenStreetMap's Kano health facilities share lineage with GRID3, and [the place benchmark](../about/place-benchmark.md) shows that from the data. And the Febrl4 person figures (precision 1.0, zero false merges, 0.877 auto-match recall) are measured on a **synthetic** corpus at the pairwise level; person resolution at scale has the run.
...
- A representation engine, not an inference engine, the full argument, with the bibliography for both halves
