...
| 3 | **Entity reference resolution** | Deciding co-reference | Shipped pairwise (gated, veto'd, [signable](../how-to/attest.md)) and at scale (`crosswalk`); transitive clustering is gated post-beta work |
...
- **Christen**, [*Data Matching*](https://doi.org/10.1007/978-3-642-31164-2) (2012). The engineering practice: the pre-processing → indexing/blocking → comparison → classification pipeline that `crosswalk` instantiates (evaluation, the fifth stage, lives in the benchmark), and the evaluation discipline (measure on labelled pairs, report both error directions) behind arche's name-frequency evaluation.
