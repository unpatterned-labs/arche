...
Use the outcome label, evidence, and provenance together. A score alone is not
an instruction to merge records.
...
1. Apply an automated action only to a contractually permitted `match` edge or
...
4. Do not cluster pairwise links into entities unless the workflow defines
