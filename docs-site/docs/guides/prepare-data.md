...
A name alone is rarely enough. Include the fields that can distinguish one entity from another.
...
Use only fields you are permitted to process. A field should be accurate enough to support a decision and traceable back to its source.
...
Accuracy only transfers when the target data, decision rule, and review process are comparable to the evaluation data.
