...
The `vnext` line set out to give a decision somewhere to live. It grew a 24-method runtime engine, thirteen tables, ten `arche case` subcommands and a SOLID Pod export before any of that was reachable from `compare()` or `reconcile()`. This release keeps the store and removes the ceremony. The short form of the reasoning is that a supplier-assurance user needs *match, remember, replay, add evidence*, and everything removed here was infrastructure for a product that had not asked for it.
