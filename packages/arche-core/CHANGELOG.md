...
- `ReviewedActionEvidence`, `record_reviewed_action_evidence(...)`, and `reassess_case(...)` establish the external-result review boundary: a permitted registry or supplier-master action yields only an Observation until a caller accepts value-free field labels. The resulting immutable Evidence can satisfy a case gap and temporarily expand the fields available to deterministic planning without changing the stored case, retaining registry values and response bodies outside Arche. Document actions still require the existing reviewed field/span path.

- Added `examples/quick_text_resolution.py` and a README quick start for `compare(text1, text2, entity="person", jurisdiction="NG", backend="regex")`. The synthetic shared-NIN/different-email example returns `same_entity` with `hold`, illustrating that resolver identity evidence is not an automatic merge or downstream action.

- Added `CaseProgress`, `assess_case_progress(...)`, and `ArcheEngine.get_case_progress(...)`: a read-only, deterministic case controller that derives the next safe move from immutable history. It distinguishes planning, caller-controlled action execution, document/external evidence review, qualified method approval, policy application, and caller-owned release execution; it cannot run tools or resolvers, create Evidence, or update entity memory. Simulated Pod consent explicitly remains a request for independent evidence rather than a resolution signal.

- Added `arche.runtime.pod_simulation` and `examples/two_pod_review.py`: an offline bilateral consent experiment using the existing case planner, persisted requester approval, session-scoped responder grants, expiry/revocation/audience checks, durable attempt tracking and immutable response Observations. Wire metadata is limited to random session/request IDs, fixed scope/purpose and consent status. Consent is excluded from independent identity evidence counts for policy release and proposal promotion. This is a permission/workflow simulation, not a SOLID authentication or matching implementation; the new `POD_SIMULATION.md` documents tests, disclosure limits and the remaining agentic MVP milestones.

- `arche.export.solid.solid_resolution_assertion(...)` and `arche case export-solid CASE_ID DECISION_ID` project an already-recorded vNext receipt into a PII-free, case-bound JSON-LD `ResolutionAssertion` for a caller-managed SOLID Pod. The projection uses fresh salted aliases for its local references and rejects cross-case receipt selection; it deliberately excludes document values, entity IDs, raw scores, arbitrary provenance, and `owl:sameAs`. This is an export seam, not a network client, federated matcher, identity assertion, or cryptographic linkage protocol.

- `SolidPodClient` and `SolidPublicationApproval` add the next caller-owned SOLID boundary. An application injects its authenticated transport; the runtime records an expiring policy/consent/capability approval before publication and a hash-only outcome after it. There is no bundled Pod discovery, OAuth/WebID client, secret storage, default HTTP transport, or path from a Pod response to Evidence, claims, or entity mutation.

- `resolve_documents(..., candidates=...)` and `arche resolve-documents` now compare local document-derived records with caller-owned, explicitly identified candidates through the existing resolver. Labelled supplier, distributor, estate, registration, and country fields are proposed with text spans for review, never asserted as claims. Candidate output remains proposed; when no candidate is safe to link, `DocumentReport.review()` returns a value-masked vNext `ResolutionCase` with a bounded `registry_lookup` permission and a `document_extract` permission only when a registration field is still missing. Candidate comparisons default to a 1,000-pair safety cap. `DocumentReport.persist(engine)` and `arche resolve-documents --store FILE.duckdb` opt into persisting only those value-free Observations, cases, and policy-permitted actions, idempotently; they do not acquire evidence or write a decision.

- `arche case open`, `arche case plan`, `arche case ingest`, `arche case evidence`, and `arche case review` provide a small vNext CLI front door. Opening a document records a hash-only Observation plus a policy-permitted extraction/OCR action; planning records bounded deterministic advice only after the caller declares a local capability; ingestion requires the planned action plus caller approval and turns Docling/OCR output into an immutable Observation; reviewed fields become span-cited Evidence without persisting their values; review exports a value-free case-history artifact for a client. None of these commands writes an entity decision implicitly.

- `arche case registry-lookup CASE_ID ACTION_ID --connector CONFIG.json` executes one persisted `registry_lookup` action through a caller-owned, read-only HTTPS registry or supplier-master configuration. Its declared source and policy pin must match the stored action; request values and response bodies are not written to DuckDB or the CLI artifact, which retains only a configuration hash and the resulting success or failure Observation. The connector's existing per-action cost ceiling and configured rate limit remain in force.

- `arche case review --html REVIEW.html` now emits a self-contained, value-free local inspection pane alongside its JSON artifact. It renders case uncertainty, permitted action outcomes, reviewed field citations, pending semantic proposal counts, and case history; it cannot apply a decision, embed a document, or reveal external request/response values. `arche case propose-tea` reuses exact reviewed document Evidence to create review-pending `reported_supplier`, `reported_distributor`, `reported_estate`, and `reported_operates` semantic proposals for caller-selected existing entities. It verifies the action, reviewed evidence, and review ID but does not create claims or relationships; independent-evidence acceptance policy remains the only promotion path.

- `arche version`, `arche list`, and `arche datasets` make the supported command surface, single-sourced installed version, and benchmark/review truth coverage discoverable without opening record payloads. `arche datasets` marks review packs as unlabelled rather than presenting them as complete evaluation data; `arche review template` writes a value-free outcome sheet for human adjudication.
...
- Resolution cases can now carry value-free `ResolutionIntent`; the deterministic planner assesses caller-configured `ResolutionMethod` candidates against entity type, operation, policy pin, required fields, and candidate-pair scale before recording a single budgeted method recommendation. It never imports or executes the chosen resolver.
- Added an opt-in HTTPS `HttpEvidenceConnector` for application-approved sources. It enforces a policy-pinned capability, action cost ceiling, and local rate window; it stores request/response hashes and status only, and converts transport, HTTP, cost, and rate failures into terminal immutable Observations.
- Added an explicit application/human executor boundary for released `PolicyDecision`s. Only a recorded `link` or `create` release reaches the caller-owned executor; its hash-only outcome is appended to case history and cannot be submitted twice.
- Added a resolver-execution gateway for planned methods. A caller or human must approve the exact persisted method/configuration pin and cost ceiling before a caller-owned Arche, Splink, or domain adapter can run; its hash-only output is recorded as a success or failure Observation, never as direct Evidence or an identity decision.
- Added the first reviewed resolver-artifact adapter for deterministic `arche.resolve.reconcile`. It creates per-edge vNext Evidence from a successful gateway Observation, records normal receipts/runs, and preserves resolver-output provenance without treating it as independent corroborating evidence for policy release.
- Reviewed Splink and domain-matcher artifacts now use the same case-history path: a matching successful gateway Observation and configuration pin are required, each reviewed edge becomes Evidence, and only the normal receipt and policy boundary may release an outcome.
- Added provider-neutral `DocumentIngestion` and a permitted document-action gateway for PDF, scan, image, or text parser results. It persists only document hashes and parser/OCR pins as an immutable Observation; reviewed spans and fields remain the separate Evidence boundary.
- Added opt-in `DoclingDocumentIngestionExecutor`, which invokes caller-owned `arche.doc.parse(...)` under a permitted document action and records only value-free parser/OCR provenance or a hash-only failure Observation.
- Added optional `AgentPlanAdvice`: a caller-owned agent can record its bounded interpretation and recommendation over an already persisted deterministic evidence plan, using reason codes and a hash reference for free-form reasoning. It cannot invent tools, execute work, approve methods, or mutate identity state.
- Optional Splink and domain resolution methods may now require an exact qualified benchmark result before planning. The planner pins the configuration, benchmark/dataset/evaluator identity, and hash-addressed result in history without treating a score as portable truth.
- Added pinned caller-owned Splink and domain runner adapters. They enforce resolver identity and attach executor provenance while returning the existing hash-only method-execution result, never direct decisions.
