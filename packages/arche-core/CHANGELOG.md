...
- **`arche.resolve_place_request(text, action=, sources=)`: from a sentence to verified endpoints, one question, or a refusal.** *"Send it tomorrow from 123 Maple Street to the blue gate behind Elim Pharmacy, 124 Elim Streat"* becomes an origin, a destination and a time window. `spatial_mentions` reads each endpoint's role, relation (`behind`, `opposite`, `next to`), reference (`Elim Pharmacy`, with the area after its comma as `reference_qualifier`), access hint (`the blue gate`) and address, typo-tolerantly. A `PlaceSource` turns a mention into candidates with evidence: `MasterSheet` is the caller's own list and needs nothing; `Nominatim` is opt-in and empty under `ARCHE_OFFLINE`. `Policy` (0.85 / 0.15 / 0.40, a typo-tolerant match is a question) turns candidates into `verified`, `clarification_required` -- one short question, `confirm` (*is it this one?*) or `number` (*which number on Awolowo Way?*) -- `refused`, or `missing`. The action is blocked until every endpoint the policy requires is verified. `POST /places` and the `extract_places` MCP tool return the roles; the request module is provisional in 0.9 and its fields may still move.
- **A place decision is a decision.** Every `Endpoint` carries a `decision_id` (`plc:sha256:…`) over the reading, the candidates shown, the policy, the status and pinned sources -- a master sheet pins as a content hash of its rows, a live geocoder as `live` and unreplayable by declaration. `resolve_place_request(..., store=ledger)` records each endpoint with verb `place`; `explain` and `replay` answer for it like any other. Engine pin `place_request.v2`.
- **The relation is used, not only read.** A landmark row may carry `front_bearing`, the side it faces; `behind` then favours the far side and counts the front against (`relation geometry agrees` / `disagrees`, ±0.15). Without it the relation proves proximity only, as before. Measured on the `places_v1` twin: the right door first in a landmark-only question 42% -> 75%.
- **`backend="auto"` and shipped recipes.** `reconcile(..., backend="auto")` -- the default -- hands the pair scoring to Splink when it is installed, a shipped `Recipe` says so, the records carry its columns, and the batch is at or above `AUTO_SPLINK_FLOOR` (1,000); otherwise arche's engine, exactly as before. The choice is on `result["backend"]`, not in the pins, so frozen decision ids do not move. `arche.resolve.recipes` ships `PERSON` (Febrl: 4,761 true / 0 false @0.99), `PLACE` (England schools: 278 / 3 @0.9, `auto=True`) and `PRODUCT` (Abt-Buy, title withheld: 732 / 49 @0.5, `auto=False`); each names the columns it reads, the settings it trains, the threshold and the benchmark it was held to, and `benchmark_gate.py` holds them there. Why the floor: below it the engine is the safer scorer (39 false merges at 1,215 records vs Splink's 47), above it Splink is (917 vs 103 at 6,144).
- **`arche.monitor`: a protocol for a trusted monitor.** `Question`, `Answer`, `Verdict`, `Monitor` -- a non-generative validator that scores a proposed decision and cannot be talked into anything, because there is no free-text field to talk into. `EngineMonitor` is the first implementation and the persuasion test is the contract.
- **`POST /documents`** on `arche serve`: files in (multipart), one resolved record per document out, masked unless `reveal=true`; parse, detect under the document's own jurisdiction, extract, resolve each against the others. `POST /places`, `POST /extract`, `--warm` / `ARCHE_WARM=1` and `/livez` reporting `warm`.
- **`arche mcp`, in the package.** The MCP server was a second package, `arche-mcp`, with its own release, changelog and a closed pin on `arche-core` that had to be moved by hand every minor. It was one module and an optional import. It is `arche.mcp` now, behind the `mcp` extra (`pip install "arche-core[mcp]"`), run as `arche mcp`; the `arche-mcp` command remains for configs written against the old name. Its tests run with the core's, so a core change is checked against the server before merge, which was the whole reason it shared a workspace. New: `arche mcp --transport streamable-http --host --port --path` serves the same tools over the network, with no authentication of its own -- put a proxy in front, as `deploy/` does.
- **One image, published.** `ghcr.io/unpatterned-labs/arche-core`: every parser, both models and the MCP server, warmed at build (the build fails if a model does not load), non-root, `arche` as the entrypoint so `docker run … mcp` is the server on stdio and the default is `serve`. `:edge` and `:sha-…` on main, `:<version>` and `:latest` on release tags; CI proves the entrypoint, the MCP handshake and that nothing local leaked into a layer. A dev image (`.devcontainer/`) carries every extra, the tests and the benchmarks.
- **`deploy/`**: a compose file with the service, the MCP server over streamable HTTP and a Caddy proxy that authenticates and sets `X-Arche-Caller`; the smallest arrangement that is honest about auth. Not a platform.
- **`data/synthetic`: worlds with a cause column.** `ng_supplier_v0`/`v1`, `africa_supplier_v1`, `artists_v0` (a Wikidata truth against a MusicBrainz variant list: what a name list is worth, priced by Splink at +10.7 bits) and `places_v0`/`v1` (a master sheet, requests that name it, every endpoint labelled with how the sentence rendered it). `places_v1` measured the typo rule -- off at `verified_at=0.7`, 34 wrong doors per 1,000 endpoints for eight points of automatic verification -- and found, on first contact, seven defects in the request module that are fixed in this release.
- **Performance.** The engine re-normalised its type vocabulary on every pair. Six small caches: 444 records 341 s -> 5 s, 6,144 records nine hours -> 70 s, identical decisions. The document path is 0.02 s warm without models and 0.86 s with.
...
- **`arche studio`** is in the wheel. The local reading tool moved from `tools/arche-studio/` to `arche._studio` -- the standard library's `http.server` and one HTML file, so the base install carries it at no cost -- and `pip install arche-core` is now the whole install. `--port`, `--packs DIR`, `--no-browser`; review packs and the reviewer's state are read from `./data` inside a checkout and `~/.arche/studio` elsewhere (`ARCHE_STUDIO_HOME` overrides). `python tools/arche-studio/serve.py` still runs from a checkout.
- **Explain by id, in the studio.** `arche studio --ledger FILE` (or `ARCHE_LEDGER`) gives the studio a ledger to read, and the new *Explain* pane takes a decision id -- `dec:`, `xwd:` or `red:` -- and shows on one screen what `arche decision`, `arche explain` and `arche replay` say: the verdict and its sentence, the fields sorted into supporting / refuting / missing with their bars, the shared values and the two records (masked until *show values* is ticked), and whether the engine installed now reproduces the id byte for byte or, if not, what moved. A redaction shows its spans by category with the section each fell under and never a value. `?explain=ID` in the URL opens straight onto a decision, so a CLI or service response can carry a link. `GET /api/explain?id=` and `/api/ledger` are the endpoints behind it.
- **Attested answers** (`arche.attest`). With a signing key -- `arche attest keygen FILE`, then `ARCHE_SIGNING_KEY` or `arche serve --signing-key` -- every answer over HTTP and over MCP carries an `attestation`: an Ed25519 JWS by the installation over the tool, a sha256 of the inputs as received, a sha256 of the answer, the caller (the `X-Arche-Caller` header behind an auth proxy, else the client address; `null` on stdio) and every decision id in the answer. `verify_attestation(envelope, inputs=, response=, public_key=)` and `arche attest verify` re-check it, keeping *valid* (unchanged since signing) apart from *trusted* (the verifier supplied the key) the way `arche.sign.verify` does; an edited answer, edited inputs, or a plain field rewritten around the JWS each fail by name. Nothing in the envelope is a value. `capabilities` names the signer. Spec on the *Attested answers* reference page.
- **`arche serve`**: the same verbs over HTTP, behind FastAPI (`arche-core[service]`). `POST /detect` returns offsets, categories and citations and never a value; `POST /deidentify` the masked copy with its `decision_id`; `POST /compare` the receipt; `GET /decision|explain|replay/{id}` read the ledger named by `--ledger` or `ARCHE_LEDGER`. Binds `127.0.0.1`, has no authentication, and the guide says so.
- **`arche.schema(...)` and `arche.extract(text, schema=)`: your fields, once.** A declaration -- a YAML path, a dict, or a `Declaration` -- is now also the extraction schema: `Declaration.labels()` turns the declared fields and descriptions into what GLiNER 2.5 is asked for, so a model proposes straight into the caller's field names (`vessel_id` with its description returns the IMO number, not the quota code beside it); `Declaration.model()` is the typed pydantic record it fills. The declared `kind` says which source outranks which: a phone, email, address or an identifier family arche has a validator for is taken from the validated detection first; a family it has no validator for is the model's to propose, and the evidence on the field says so. Two declared fields of one kind are never both filled with the same generic entity -- both stay unresolved instead. `Extraction.record`, `.reference(record_id)` (the canonical form under the declaration's roles) and `.pins()` (backend, model, declaration pin, `reproducible`) carry it into `compare`; `schema=` is accepted by `compare`, `reconcile`, `dedupe` and `find` as the caller's word for `decl=`. `arche.doc.extract` takes a `Declaration` beside a pydantic class and is the one implementation.

- **Abt-Buy joins the benchmark gate.** `data/scripts/benchmark_gate.py` now runs the `product_electronics` pack on Abt-Buy names (recorded in `data/er_bench/benchmark_abt_buy_result.json`: 741 true merges, 22 false, precision 0.9712) beside DBLP-ACM and Febrl 4, and checks two invariants the product pack's documentation relies on — the `spec` refutation is neutral on that corpus and the stop list is inert on it. Either breaking fails the build in its own right. The three unit tests that used to assert this (`TestBenchmarkContract`) are gone from the suite, which is 190 seconds faster for it; the small-catalogue stop-list test stays.

- **A detection benchmark.** `data/scripts/benchmark_pii.py` runs `Pipeline` over a set with exact spans and reports recall per category, false positives with the negative each landed on, and nested detections, per backend. The first set, `data/pii_bench/african_context_v0.jsonl` (CC-BY-4.0, constructed by `build_african_context_v0.py`: 240 texts, 908 spans, 540 negatives, every identifier validator-checked at build time, names split into lexicon and held-out pools), gives the first numbers the detectors have ever had: `basic` 0.818 recall at 4 ms a text, `gliner2-pii` 0.986 at ~0.8 s, false positives entirely on order numbers and bare ten-digit references — 35 read as phones, 25 as Kenyan ids, 8 as driving licences — which is the cue-gate audit with a count against each pattern. The data card names what the set is not (text from the wild) and why ai4privacy's 300k was not used (academic-only, no derivatives). Results on the benchmarks page under *Detection*.

### Fixed

- **The cue-gate audit**, each row with the count the detection benchmark put against it (240 texts, 540 negatives). A bare run of eight digits was a phone number somewhere in Africa (35 order numbers): the phone parser's every-prefix fallback now runs only for a number written with its trunk zero. Seven or eight bare digits were a Kenyan national id or NHIF number (25): both patterns are cue-anchored like Huduma — `ID No.`, `national ID`, `NHIF` — at 0.85 instead of 0.40–0.45 bare. `INV-3702838222` was a driving licence from a state called INV (8): a three-letter document prefix is not a state code. The model's ten-digit passports (18) and eight-digit phones (17) were shapes that cannot be what they were labelled: a passport carries a letter, a phone a trunk zero, a plus, or nine digits. `basic` goes from 28 false positives per 100 texts to 3, the model from 55 to 24, and recall does not move on either.
- The PII model is now asked for `city`, `state_or_region`, `postal_code` and `country` beside `address`. Asked for `address` alone it found none of 548 city/state/country/postcode spans on a European sample; asked for the four it recovered 0.81 of them at 0.81 precision, and name precision did not move. Found by the detection benchmark, on a set that could not be used for anything but finding it.
- *Given*, *Best*, *Holder*, *Law* were names. The lexicon holds them as surnames; in English prose they are *given name:*, *best regards*, *account holder* — 30 of 91 name false positives on 300 texts. They join the stop list with *say*, *family*, *person*, *immediate*.
- *Next of kin* was a location. The gazetteer alias *Kin* (Kinshasa) matched case-insensitively, 32 times in 240 benchmark texts. A lowercase gazetteer match shorter than five characters is now treated as a word; *Kano* and *lagos* are still found.
...
- **The `arche-mcp` package** (`packages/arche-mcp`, never published). Its code is `arche.mcp`, its tests are the core's, its changelog is this one. `pip install arche-mcp` was never a thing that worked; `pip install "arche-core[mcp]"` is.
...
The `vnext` line set out to give a decision somewhere to live. It grew a 24-method runtime engine, thirteen tables, ten `arche case` subcommands and a SOLID Pod export before any of that was reachable from `compare()` or `reconcile()`. This release keeps the store and removes the ceremony.

It is also the first release without an alpha suffix. Not because the API is frozen — it is pre-1.0 and minor versions may still move it — but because the surface is now small enough to be stated on one page and every claim on that page is executed by the test suite. The `4 - Beta` classifier says the same thing. The short form of the reasoning is that a supplier-assurance user needs *match, remember, replay, add evidence*, and everything removed here was infrastructure for a product that had not asked for it.

### Added

- **`arche.attach("duckdb:///...")` opens a `Ledger`**, and every verb — `compare`, `reconcile`, `dedupe`, `find`, `resolve_documents` — takes `store=ledger`. The receipt is unchanged; it is additionally recorded with the inputs it was made from. The optional `arche-core[ledger]` extra installs DuckDB (`[runtime]` still works and means the same thing).
- `ledger.decision(id)` returns the receipt as recorded; `ledger.explain(id)` splits it into supporting, refuting and missing fields with the agreed values; `ledger.replay(id)` makes the decision again under the installed engine and reports `reproduced`, or names every factor and pin that moved; `ledger.history(record_id)` lists what touched a record.
- Linked decisions build entities: `ledger.entities()` shows which records the verdicts hold together, the attributes they all agree on and the ones they disagree about. Three texts that pairwise say `same_entity` (two of them `hold`) become one person sharing a national id with a conflicting email. Entities merge when a later decision links them; nothing is deleted.
- `ledger.cases()` lists pairs still at `review` with what would settle them; `ledger.observe(record_id, {...})` adds evidence about a record, re-decides its open pairs and records which decision each new receipt supersedes. That is the plan → act → observe → update loop, with the caller choosing the action.
- **Names are read offline.** The 13,342-name lexicon of African given and family names (CC-BY-4.0, Wikidata/ParaNames-derived) now ships in the wheel as `arche/_data/african_names_v1.jsonl.gz` (55 KB). `pip install arche-core` used to detect 118 names; *Adesola Okonkwo* was not among them. `detect_names` matches lexicon names token by token when capitalised; the new `person_spans()` merges adjacent name tokens (an initial allowed between them) into one span and requires at least two; the `regex` extractor emits `PERSON` from it. A text-derived person record therefore carries a name, which corroborates a shared id: the three-text example moves from `same_entity / hold` to `same_entity / merge`. The lexicon holds Anglo names common in Africa (*Jane*, *Smith*, *Grace*), so the `names` pack is no longer "African-only"; the calibration test's negative example is now *Björn Svensson*. The lexicon loader also stopped depending on the working directory, which was the actual cause of a `names` test that passed from the repository root and failed from `packages/arche-core`.
- `EntityView.held_together_by` says whether an entity is `direct` (every pair of members itself decided the same) or `transitive` (linked only through a third record), the word `dedupe()` already used for its clusters; `weak_links` names the records whose removal would split a transitive entity and `bridges` the decisions that would. Measured on DBLP-ACM and Febrl 4 with complete truth: every cross-cluster merge on DBLP-ACM and 184 of 195 on Febrl are transitive, so `direct` is a usable guarantee. `data/scripts/benchmark_entity_formation.py`; numbers on the benchmarks page.
- **Association analysis.** `ledger.path(record_a, record_b)` returns the chain of decisions that makes two records one entity — the explanation for a transitive link, the pair a user looks at and asks *why*; `ledger.graph()` is the association graph as a `networkx.Graph`; `arche path A B` from the shell. The guide *Association analysis* walks a four-record name-and-address change end to end. `decision_id` is the handle to all of it: `decision`, `explain`, `replay`, `path`, `history`, `events` each give back a step of the reasoning by id.
- **Transitive matching.** `ledger.resolve(record, entity_type=...)` resolves a new record — a dict, or text for the person pack — against the entities the ledger already holds: compared with every stored record of its type, grouped by entity, decided at the entity level. `found` links it; `review` opens the nearest pairs as cases; `ambiguous` (members of two entities matched) and `conflict` (contradicts an identifier the entity agrees on — `kind: id` fields only, never an email or address) record the decisions but withhold the link and say so in an event; `not_found` stores it alone. `entity_evidence` compares the record with the entity as a whole, built only from attributes its members do not dispute. `arche resolve --text|--record` from the shell; the `resolve` tool in `arche-mcp`.
- **Benchmark gate.** `data/scripts/benchmark_gate.py` runs DBLP-ACM (year refutes) and Febrl 4 (name + address) and compares them with the committed result files; `.github/workflows/arche-core-benchmarks.yml` runs it on every change to the resolver. A precision drop over 0.5 points or a true-merge drop over 0.5% fails; an improvement is reported so the baseline is raised on purpose with `--update`.
- **Febrl 4 drift, bisected.** The name-and-address run records 484 false merges where 282 were recorded on 2026-08-17; true merges are identical. One commit, `e9cc9a8` (0.5.0a1, conjunction blocking), nearly doubled candidate pairs on Febrl without adding a true pair, and the scorer merged 202 of the newcomers — identical common names with 0.7-ish addresses. Recorded on the benchmarks page and in `bench_febrl_result.json`; not reverted, because the same change recovered 27,055 true pairs on `historical_50k`. The scorer, not the blocker, owes the fix.
- `ledger.read(source)` returns rows from a table in the ledger's DuckDB file, a `.csv` / `.parquet` / `.json` path, or a `SELECT`, ready for a verb — attaching is the door to the data as well as to the decisions.
- `would_resolve(..., threshold=)` recognises a pair held at `review` because its score fell short — one shared rare word and nothing else — instead of misreading it as a hard-constraint conflict and listing no fields. `ledger.cases()` passes the run's threshold, so an open organisation case now says `registration_id, entity_class, address, lat + lon` rather than nothing.
- `Ledger.record_batch` writes in bulk: one existence query and one multi-row insert per table, memberships resolved by an in-memory union-find. 3,000 edges record in 2.4 s where the per-edge path took 21–28 s (0.8 ms an edge, from 9).
- `Ledger.observe` re-decides a batch edge through the pack's pair engine (the new receipt's pins say so), and reuses the untouched side's stored record instead of minting a second one under a different content address — which had made the re-decided pair a new entity rather than joining the one it came from.
- `resolve_documents(..., candidates=...)` and `arche resolve-documents` compare document-derived records with explicitly identified caller-owned candidates under a 1,000-pair default cap. `DocumentReport.review()` is the masked hand-off; `unlinked()` names the documents no verdict tied to anything; `--store FILE.duckdb` records everything in a ledger and adds a value-free entity summary to the JSON.
- `resolve_documents(..., extraction_backend="regex")` and `entity_backend="regex"` give a deterministic, model-free extraction path; `arche resolve-documents` defaults to it so a first run does not begin with a model download.
- `compare(text1, text2, entity="person", jurisdiction="NG", backend="regex")` compares two pieces of text directly; `examples/quick_text_resolution.py` is the runnable form. `detect_pii(text, backend="regex")` no longer initialises Presidio for a deterministic request.
- **The ledger from the shell.** `arche compare --text A B [--store FILE | --record]` asks the pairwise question of two pieces of text and can record the receipt; `arche decision`, `explain`, `replay`, `entities`, `cases` read a ledger and `arche observe RECORD_ID --evidence '{...}'` adds evidence and re-decides. `--store` also lands on the file form of `arche compare`. `ARCHE_LEDGER` names the default file; values are masked unless `--reveal`; `--json` everywhere.
...
- The `arche.export.solid` JSON-LD assertion, `SolidPodClient`, `SolidPublicationApproval`, `arche.runtime.pod_simulation` and `arche case export-solid`. SOLID Pods return only when two independently administered parties need a privacy-preserving exchange and a threat model has been written.
- The approval bureaucracy: `PinnedResolutionMethodExecutor`, `PolicyDecisionExecutor`, `ResolutionMethodApproval`, `ResolutionDecisionPolicy`, `ProposalAcceptancePolicy`, reviewed-artifact adapters, `BenchmarkResultBundle` qualification gates, `AgentPlanAdvice`, `CaseProgress`, `reassess_case`, `HttpEvidenceConnector`, `DeterministicResolutionPlanner`, claim and relation proposals. The caller, human or agent, holds authority over what to run and what to release; the store records what happened.
- `arche case open/plan/ingest/evidence/review-action/propose-tea/registry-lookup/progress/review`, `report.case_review_pane`, `DocumentReport.persist()`, and the `arche.runtime` / `arche.store` packages (24-method engine, thirteen tables) that `arche.ledger` replaces with one class and five tables.
- Notebooks 23 and 24 and `examples/two_pod_review.py`, which demonstrated the removed surface.
