...
- `arche case open`, `arche case plan`, and `arche case review` provide a small vNext CLI front door. Opening a document records a hash-only Observation plus a policy-permitted extraction/OCR action; planning records bounded deterministic advice only after the caller declares a local capability; review exports a value-free case-history artifact for a client. None of these commands parse documents, run a resolver, or write an entity decision implicitly.
