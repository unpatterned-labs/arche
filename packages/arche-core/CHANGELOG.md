...
- `EntityView.held` says whether an entity is `direct` (every pair of members itself decided the same) or `transitive` (linked only through a third record). Measured on DBLP-ACM and Febrl 4 with complete truth: every cross-cluster merge on DBLP-ACM and 184 of 195 on Febrl are transitive, so `direct` is a usable guarantee. `data/scripts/benchmark_entity_formation.py`; numbers on the benchmarks page.
