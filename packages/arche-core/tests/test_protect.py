...
def test_detect_phone_pii():
    pii = detect_pii("Call me at +234 803 555 7890", backend="regex")
    assert len(pii) >= 1
    assert any(p.pii_type == "PHONE_NUMBER" for p in pii)


def test_detect_email_pii():
    pii = detect_pii("Email: janet@example.com", backend="regex")
    assert len(pii) >= 1
    assert any("EMAIL" in p.pii_type for p in pii)


def test_detect_nigerian_nin():
    pii = detect_pii("My NIN is 12345678901", backend="regex")
...
def test_detect_pii_rejects_unknown_backend():
    with pytest.raises(ValueError, match="backend"):
        detect_pii("Email: janet@example.com", backend="model")
