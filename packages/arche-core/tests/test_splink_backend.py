...
"""`reconcile(backend="splink")`: contract, not accuracy.
...
def result():
    return reconcile(_A, _B, entity="person", id_field="id", backend="splink",
...
class TestThePins:
    ...
    def test_a_different_corpus_pins_differently(self, result):
        ...
        other = reconcile(bigger, _B, entity="person", id_field="id",
    ...
    def test_the_blocking_pin_is_stable_across_runs(self, result):
        ...
        model's SQL now.
        """
        again = reconcile(_A, _B, entity="person", id_field="id",
                          backend="splink", splink_settings=settings(),
                          threshold=0.99)
...
class TestFailureContract:
    ...
    def test_an_unknown_backend_is_refused(self):
        with pytest.raises(ValueError, match="unknown backend"):
            reconcile(_A, _B, entity="person", id_field="id", backend="nope")
    ...
    def test_settings_are_required(self):
        ...
        with pytest.raises(SplinkBackendError, match="needs a Splink config"):
            reconcile(_A, _B, entity="person", id_field="id", backend="splink")
    ...
    def test_a_threshold_is_required(self):
        ...
        with pytest.raises(SplinkBackendError, match="needs threshold="):
            reconcile(_A, _B, entity="person", id_field="id",
                      backend="splink", splink_settings=settings())
    ...
    def test_deriving_without_a_pack_is_refused(self):
        with pytest.raises(SplinkBackendError, match="nothing to derive"):
            reconcile(_A, _B, id_field="id", backend="splink",
...
class TestColumnTypes:
    ...
    def test_a_geo_run_completes(self):
        ...
        res = reconcile(_GEO, _GEO, id_field="id", backend="splink",
...
class TestTheGate:
    ...
    def test_can_be_asked_for(self):
        gated = reconcile(_A, _B, entity="person", id_field="id",
                          backend="splink", splink_settings=settings(),
