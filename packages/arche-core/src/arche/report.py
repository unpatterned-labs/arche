...
def case_review_pane(artifact: dict[str, Any]) -> str:
    """Render a self-contained, value-free local pane for a vNext case artifact.

    The pane deliberately consumes the JSON emitted by ``arche case review``.
    It is an inspection surface only: source documents and reviewed values stay
    with the caller, and no button in the artifact can apply a decision.
    """
    case = artifact.get("case") if isinstance(artifact.get("case"), dict) else {}
    actions = artifact.get("permitted_actions")
    evidence = artifact.get("reviewed_evidence")
    history = artifact.get("history")
    action_results = artifact.get("action_observations")
    actions = actions if isinstance(actions, list) else []
    evidence = evidence if isinstance(evidence, list) else []
    history = history if isinstance(history, list) else []
    action_results = action_results if isinstance(action_results, list) else []
    by_action = {
        item.get("action_id"): item.get("observation", {})
        for item in action_results
        if isinstance(item, dict) and isinstance(item.get("observation"), dict)
    }

    def table(headers: str, items: list[str], empty: str) -> str:
        if not items:
            return f'<p class="mute">{_esc(empty)}</p>'
        return f"<table><thead>{headers}</thead><tbody>{''.join(items)}</tbody></table>"

    action_rows = []
    for action in actions:
        if not isinstance(action, dict):
            continue
        result = by_action.get(action.get("action_id"), {})
        outcome = result.get("provenance", {}).get("outcome", "not acquired")
        action_rows.append(
            "<tr>"
            f"<td>{_esc(action.get('action_type', ''))}</td>"
            f"<td>{_esc(action.get('source_id', ''))}</td>"
            f"<td>{_esc(action.get('policy_pin', ''))}</td>"
            f"<td>{_esc(action.get('max_cost', ''))}</td>"
    ...
    for item in evidence:
        ...
        if not isinstance(provenance, dict):
            provenance = {}
        citation = ", ".join(
            f"{name}={provenance[name]}" for name in ("page", "span") if name in provenance
        ) or "no page/span"
        evidence_rows.append(
            "<tr>"
            f"<td>{_esc(provenance.get('field', ''))}</td>"
            f"<td>{_esc(provenance.get('confidence', ''))}</td>"
            f"<td>{_esc(citation)}</td>"
            f"<td>{_esc(item.get('observation_id', ''))}</td>"
            "</tr>"
        )
    proposal_rows = []
    for event in history:
        if not isinstance(event, dict) or event.get("event_type") != "reviewed_document_proposals":
            continue
        ...
        proposal_rows.append(
            "<tr>"
            f"<td>{_esc(len(provenance.get('claim_proposals', [])))}</td>"
            f"<td>{_esc(len(provenance.get('relation_proposals', [])))}</td>"
            f"<td>{_esc(provenance.get('review_id', ''))}</td>"
            "</tr>"
        )
    event_rows = [
        "<tr>"
        f"<td>{_esc(event.get('event_type', ''))}</td>"
        f"<td>{_esc(len(event.get('references', [])))}</td>"
        f"<td>{_esc(event.get('recorded_at', ''))}</td>"
        "</tr>"
        for event in history
        if isinstance(event, dict)
    ]
    candidate_ids = case.get("candidate_entity_ids", []) if isinstance(case, dict) else []
    uncertainty = case.get("uncertainty", {}) if isinstance(case, dict) else {}
    candidates = ", ".join(candidate_ids) if isinstance(candidate_ids, list) else ""
    action_table = table(
        "<tr><th>Action</th><th>Source</th><th>Policy</th><th>Cost ceiling</th>"
        "<th>Result</th></tr>",
        action_rows,
        "No permitted actions.",
    )
    evidence_table = table(
        "<tr><th>Field</th><th>Confidence</th><th>Citation</th><th>Observation</th></tr>",
        evidence_rows,
        "No reviewed fields.",
    )
    proposal_table = table(
        "<tr><th>Claims</th><th>Relationships</th><th>Review</th></tr>",
        proposal_rows,
        "No semantic proposals.",
    )
    history_table = table(
        "<tr><th>Event</th><th>References</th><th>Recorded</th></tr>",
        event_rows,
        "No case events.",
    )
    css = f"""
...
.sub, .mute {{ color: {_MUTE}; font-size: 13px; }}
.grid {{
  display: grid;
...
.label {{ color: {_MUTE}; font-size: 11px; text-transform: uppercase; }}
...
th {{ color: {_MUTE}; font-size: 11px; text-transform: uppercase; }}
...
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>arche case review</title><style>{css}</style></head><body><main>
<h1>Resolution case review</h1>
<p class="sub">Value-free local inspection artifact. It cannot apply a decision.</p>
<div class="grid"><div class="card"><div class="label">Case</div>
<code>{_esc(case.get('case_id', ''))}</code></div>
