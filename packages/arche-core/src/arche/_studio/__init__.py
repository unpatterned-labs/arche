...
def _home() -> Path:
    """Where packs and state live.

    A checkout with ``data/review_packs`` keeps the layout it always had; a
    wheel on someone's laptop gets ``~/.arche/studio``. ``ARCHE_STUDIO_HOME``
    names another place, for a shared machine or a container volume.
    """
    env = os.environ.get("ARCHE_STUDIO_HOME")
    if env:
        return Path(env).expanduser()
    checkout = Path.cwd() / "data"
    if (checkout / "review_packs").exists():
        return checkout
    return Path.home() / ".arche" / "studio"


HOME = _home()
PACKS = Path(os.environ.get("ARCHE_STUDIO_PACKS") or HOME / "review_packs").expanduser()
...
#: CLI and ``arche serve`` read it; ``--ledger FILE`` overrides. ``None`` means
#: the pane explains that there is nothing to read from, rather than hiding.
LEDGER_URI: str | None = os.environ.get("ARCHE_LEDGER") or None
_LEDGER: dict = {}
...
def _ledger():
    """The attached ledger, opened once. Raises with the fix when there is none."""
    if not LEDGER_URI:
        raise ValueError(
            "no ledger to read from: start the studio with --ledger FILE.duckdb "
            "or set ARCHE_LEDGER to the file your decisions were recorded in"
        )
    uri = LEDGER_URI if "://" in LEDGER_URI else f"duckdb:///{LEDGER_URI}"
    if _LEDGER.get("uri") != uri:
        from arche.ledger import attach

        _LEDGER.clear()
        _LEDGER.update(uri=uri, book=attach(uri))
    return _LEDGER["book"]


def _ledger_status() -> dict:
    """What the Explain pane should say before anyone types an id."""
    return {"attached": bool(LEDGER_URI), "uri": LEDGER_URI}


def _explain(decision_id: str, reveal: bool = False) -> dict:
    """One decision by id: what was decided, why, and whether it holds today.

    The same three answers ``arche decision``, ``arche explain`` and
    ``arche replay`` give, on one screen. Values are masked unless the reviewer
    asks, the way the CLI masks them: the decision is the object here, and a
    page that echoes every national id it was asked about has changed subject.
    A replay that cannot run (the call had arguments the ledger could not
    store) is reported as such rather than failing the whole view.
    """
    from arche.cli import _decision_payload, _record_payload, _shown

    decision_id = (decision_id or "").strip()
    if not decision_id:
        raise ValueError("paste a decision id first (dec:sha256:…, xwd:…, red:…)")
    book = _ledger()
    try:
        decision = book.decision(decision_id)
    except KeyError:
        raise ValueError(f"no decision {decision_id!r} in this ledger") from None
    why = book.explain(decision_id)
    if "shared" in why:
    ...
    if decision.verb != "deidentify":
        for record_id in (decision.record_a, decision.record_b):
            try:
                records.append(_record_payload(book.record(record_id), reveal))
            except KeyError:
                records.append({"record_id": record_id, "missing": True})
    try:
        again = book.replay(decision_id)
        replay = {"reproduced": again.reproduced, "changed": again.changed,
                  "now": {k: v for k, v in again.now.items() if k != "pins"}}
    except ValueError as exc:
        replay = {"unreplayable": str(exc)}
    return {"decision": _decision_payload(decision, reveal), "why": why,
            "records": records, "replay": replay, "reveal": reveal}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            elif u.path == "/api/ledger":
                self._json(_ledger_status())
            elif u.path == "/api/explain":
                q = parse_qs(u.query)
                self._json(_explain(q.get("id", [""])[0],
                                    reveal=q.get("reveal", ["0"])[0] in ("1", "true")))
...
def main(argv: list[str] | None = None) -> int:
    """``arche studio [--port N] [--packs DIR] [--no-browser]``."""
    global PACKS, LEDGER_URI
    import argparse
    ...
    if args.ledger:
        LEDGER_URI = args.ledger
    PACKS.mkdir(parents=True, exist_ok=True)
    if not (HERE / "index.html").exists():
        print("index.html is missing from the arche package", file=sys.stderr)
        return 1
    try:
        srv = Studio(("127.0.0.1", args.port), Handler)
    ...
    print(f"  state         ->  {STUDIO_STATE}")
    print(f"  ledger        ->  {LEDGER_URI or 'none (--ledger FILE or ARCHE_LEDGER)'}")
