...
| `12_review_feedback_and_evaluation.ipynb` | Inline, fictional facility records | From repository root: `uv run --package arche-core jupyter notebook examples/notebooks/12_review_feedback_and_evaluation.ipynb` | 4 code cells completed in 13.1 seconds. The example produced one `match`, one `review`, a `different` reviewer outcome, and an evaluation report. | Demonstrates the review-outcome and evaluation contract. It is not an accuracy benchmark. |
