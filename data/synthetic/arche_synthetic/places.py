...
class PlaceGenerator:
    ...
    def _render(self, row: dict, rendering: str) -> tuple[str, str | None]:
        ...
        if rendering in ("landmark", "landmark_only"):
            ...
            rel = self.rng.choice(_RELATIONS)
    ...
    def _rendering(self) -> str:
        return self.rng.choices(list(RENDERINGS), weights=list(RENDERINGS.values()))[0]
...
def build(*, seed: int = 42, sheet: int = 500, requests: int = 1000,
          out: Path | None = None, generator_version: str = "0.0.1"
          ) -> tuple[PlaceGenerator, dict[str, Any]]:
    gen = PlaceGenerator(seed, sheet=sheet, requests=requests)
    gen.build()
    ...
    if out is not None:
        ...
        manifest = {
            "world_id": gen.world.world_id, "world_pack": WORLD_PACK, "seed": seed,
            "generator_version": generator_version, "schema_version": 0,
            "counts": {"sheet_addresses": len(gen.sheet), "landmarks": len(gen.landmarks),
                       "requests": len(gen.requests), "endpoints": len(truth_rows),
                       "twins": len(gen._twins), "renderings": dict(sorted(counts.items()))},
...
class PlaceBenchmark:
    ...
    def load(cls, world: Path) -> PlaceBenchmark:
        ...
        sheet = pq.read_table(world / "sheet.parquet").to_pylist()
...
__all__ = ["COUNTRIES", "OUTCOMES", "RENDERINGS", "WORLD_PACK", "Endpoint",
           "PlaceBenchmark", "PlaceGenerator", "Request", "build"]
